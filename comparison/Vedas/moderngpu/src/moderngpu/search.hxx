...
// Adapter overload to make VEDAS' int-based call compile.
template <typename needles_it, typename haystack_it, typename comp_t>
mem_t<typename std::iterator_traits<needles_it>::value_type>
merge_path_partitions(needles_it needles, int num_needles,
                      haystack_it haystack, int num_haystack,
                      int spacing, comp_t comp, context_t& context) {
  // Forward to the main implementation that uses 64-bit counters.
  return merge_path_partitions<empty_t, needles_it, haystack_it, comp_t>(
      needles,
      static_cast<int64_t>(num_needles),
      haystack,
      static_cast<int64_t>(num_haystack),
      static_cast<int64_t>(spacing),
      comp,
      context);
}
